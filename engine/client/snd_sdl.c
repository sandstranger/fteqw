...
#if defined(SDL_HINT_AUDIO_DRIVER)
#define FTE_AUDIO_DRIVER_HINT SDL_HINT_AUDIO_DRIVER
#else
#define FTE_AUDIO_DRIVER_HINT SDL_HINT_AUDIODRIVER
#endif
...
static qboolean SSDL_InitAudio(void)
{
    ...
    if (!inited)
    {
        ...
        if (api_level >= 26)
            SDL_SetHint(FTE_AUDIO_DRIVER_HINT, "aaudio");
        else
            SDL_SetHint(FTE_AUDIO_DRIVER_HINT, "opensles");
        SDL_SetHint("SDL_AUDIO_FORMAT", "S16");
#endif
