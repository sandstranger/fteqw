...
//nettest Patch 36 Part B: register a per-bone hitbox on an alias/IQM model (called from the addmodelhitbox builtin).
extern qboolean	Mod_AddHitbox					(struct model_s *model, const char *bonename, int hitgroup, const float *mins, const float *maxs);
